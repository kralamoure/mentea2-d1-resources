...
def downloadRelease(version):
    ...
    if OUT_REMASTERED:
        downloadReleasePart(release["remastered"])
    else:
        downloadReleasePart(release["classic"])
    downloadReleasePart(release[OUT_PLATFORM])

def downloadClient():
    version = getVersion()
    subVersions = version.split("_")[1].split("-")[0].split(".")
    print("version: " + subVersions[0] + "." + subVersions[1] + "." + subVersions[2])
...
def cleanClient():
    ...
    for filename in os.listdir(clientPath):
        ...
        if os.path.isdir(filePath):
            ...
            if filename == "D1ElectronLauncher.html":
                os.remove(filePath)
    docsPath = os.path.join(clientPath, "data", "docs")
    for filename in os.listdir(docsPath):
        ...
        if os.path.isfile(filePath):
            ...
            if filenameSplit[0] not in OUT_LANGS:
                os.remove(filePath)
    moveFiles(clientPath, OUT_FOLDER)
